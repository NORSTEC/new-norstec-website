...
export default function Map({ organizations }: MapProps) {
  const orgs = (organizations ?? []).filter(
    (org): org is Organization & { mapPosition: { x: number; y: number } } => !!org.mapPosition
  );
  ...
  const baseCircleClasses = "cursor-pointer outline-0 hover:stroke-moody hover:z-40 transition-all";
        ...
        >
          ...
          {orgs.map((org) => (
            <circle
              ...
              onMouseLeave={() => setHoveredId((prev) => (prev === org._id ? null : prev))}
              onFocus={() => setHoveredId(org._id)}
              onBlur={() => setHoveredId((prev) => (prev === org._id ? null : prev))}
              tabIndex={0}
            ></circle>
          ))}
        </svg>
