...
function getPriorityColumns(priority: number) {
...
export default function SectionSponsors({ section, className = "" }: Props) {
  ...
  const groupedSponsors = useMemo(() => {
    const groups = new Map<number, LayoutSponsor[]>();

    sponsors.forEach((sponsor) => {
      const current = groups.get(sponsor.priority) ?? [];
      current.push(sponsor);
      groups.set(sponsor.priority, current);
    });
  ...
  return (
    <section className={`section desktop-container ${className}`}>
      <div className="flex h-full flex-col">
        ...
        <div className="pt-5 xl:hidden overflow-y-auto pr-1">
          <div className="space-y-4">
            {groupedSponsors.map(([priority, items]) => {
              const columns = getPriorityColumns(priority);

              return (
                <div
                  key={`priority-mobile-${priority}`}
        ...
        <div className="pt-5 justify-center items-center h-full hidden xl:flex">
          <div
          ...
          >
            {sponsors.map((sponsor) => {
              ...
              const card = (
                ...
                >
                  <div className="flex h-full w-full items-center justify-center rounded-xl transition-all p-2 md:p-3">
                    {logoSrc ? (
                      <div className="relative h-full w-full">
                        <Image
                          src={logoSrc}
